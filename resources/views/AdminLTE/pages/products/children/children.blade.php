                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">Products Children Image</h3>
                                        <a href="{{ route('child.product.create') }}" class="btn btn-info float-right ms-5"
                                            type="button">Add</a>
                                    </div>
                                    <!-- /.card-header -->
                                    <div class="card-body">
                                        <table id="example2" class="table table-bordered table-hover">
                                            ...
                                            <tbody>
                                                @forelse ($childImages as $child)
                                                    <tr>
                                                        <td>{{ $child->id}}</td>
                                                        <td><img src="{{ asset('images/' . $child->image) }}"
                                                                alt="{{ $child->name }}" width="50"></td>
                                                        <td class="flex flex-row text-center gap-3">
                                                            {{-- Block Update --}}
                                                            <a href="{{ route('child.product.edit', $child->id) }}"
                                                            ...
                                                            {{-- Block Delete --}}
                                                            <form method="POST"
                                                                action="{{ route('child.product.distroy', $child->id) }}">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button
                                                ...
                                                @empty
                                                    <tr>
                                                        <td colspan="9">No products Image children found.</td>
                                                    </tr>
                                                @endforelse
                                            </tbody>
                                        </table>
